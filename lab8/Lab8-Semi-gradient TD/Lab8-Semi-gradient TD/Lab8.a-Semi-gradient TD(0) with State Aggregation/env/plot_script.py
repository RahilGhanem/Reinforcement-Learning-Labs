...
def plot_result(agent_parameters, directory):
    ...
    for num_g in agent_parameters["num_groups"]:
        ...
        for step_size in agent_parameters["step_size"]:
            ...
            # plot1
            filename = 'V_TD_agent_agg_states_{}_step_size_{}'.format(
                num_g, step_size).replace('.', '')
            current_agent_V = np.load('{}/{}.npy'.format(directory, filename))

            plt1_x_legend = range(1, len(current_agent_V[:]) + 1)
            graph_current_agent_V, = ax[0].plot(
                plt1_x_legend, current_agent_V[:], label="approximate values: state aggregation: {}, step-size: {}".format(num_g, step_size))
            plt1_agent_sweeps.append(graph_current_agent_V)

            # plot2
            filename = 'RMSVE_TD_agent_agg_states_{}_step_size_{}'.format(
                num_g, step_size).replace('.', '')
            current_agent_RMSVE = np.load(
                '{}/{}.npy'.format(directory, filename))

            plt2_x_legend = range(1, len(current_agent_RMSVE[:]) + 1)
            graph_current_agent_RMSVE, = ax[1].plot(
                plt2_x_legend, current_agent_RMSVE[:], label="RMSVE average: state aggregation: {}, step-size: {}".format(num_g, step_size))
        ...
        plt2_xticklabels = range(0, 2100, 200)
        ...
        plt.tight_layout()
        plt.suptitle("{}-State Aggregation".format(num_g),
