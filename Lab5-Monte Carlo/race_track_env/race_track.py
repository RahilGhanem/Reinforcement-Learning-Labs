...
class RaceTrack(Env):
    ...
    def __init__(self, track_map: str, render_mode: str = None, size: int = 2):
        ...
        self.window_size = (
            self.window_size[1] * self.size, self.window_size[0] * self.size)
    ...
    def step(self, action):
        ...
        if temp_y_acc < -4:
            temp_y_acc = -4
        if temp_y_acc > 0:
            temp_y_acc = 0  # Avoid the car from going backward
        if temp_x_acc < -4:
            temp_x_acc = -4
        if temp_x_acc > 4:
            temp_x_acc = 4
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                if cell_val == FINISHING:
                    fill = (235, 52, 52)
                    pygame.draw.rect(
                        self.window, fill, (col * self.size, row * self.size, self.size, self.size), 0)
                # Draw starting cells
                ...
                pygame.draw.rect(
                    self.window, color, (col * self.size, row * self.size, self.size, self.size), 1)
